/**
 * Wishlist grid — shared by /wishlist and /account/liked.
 */
...
interface WishlistContentProps {
  /** When true, hide the top hero row (account embeds its own context). */
  compact?: boolean;
}

export function WishlistContent({ compact = false }: WishlistContentProps) {
  ...
  return (
    <div className={compact ? '' : 'min-h-screen bg-[#F5F0EB] pb-32 md:pb-24'}>
      <div className={compact ? '' : 'container px-5 pt-10 md:pt-14'}>
        {!compact && (
        ...
        {compact && (
          <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <Link
              to="/collection"
              className="inline-flex items-center justify-center gap-2 rounded-full bg-[#0D0D0D] text-white px-5 py-2 text-xs font-bold uppercase tracking-[0.12em] w-fit"
            >
              Shop more
